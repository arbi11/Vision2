# -*- coding: utf-8 -*-
"""
Created on Wed Mar 14 21:49:10 2018
...
a0 = np.linalg.det(s) ** -.5 ** (2 * np.pi) ** (-X.shape[1]/2.)

a1 = np.exp(-.5 * np.einsum('ij, ij -> i', X - m, np.dot(np.linalg.inv(s) , (X - m).T).T))

a2 = np.einsum('ij, ij -> i', X - m, np.dot(np.linalg.inv(s) , (X - m).T).T) 

b =  np.dot(np.linalg.inv(s) , (X - m).T).T 

c = np.dot(np.linalg.inv(s) , (X - m).T)
